...
//-----------------------------------------------------------------------------
	...
	signFile: function() {
		...
		var url = searchParams.get('doc');
		var fileName = searchParams.get('doc').split("/").slice(-1).pop().split(".docx")[0];
		var baseFileName = searchParams.get('doc').split("/").slice(-1).pop();
		...
		try {
			...
			httpRequest.onload = function() {
				...
				if (httpRequest.status == 200) {
					...
					alert("File download issue");
				}
			};

			httpRequest.onerror = function() {
				alert("File download issue 2");
			};
